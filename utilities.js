 ...
 */
 My.InjectCSSOld = function( cssObj, returnGeneratedCSS ) {
  ...
  for ( cssSelector in cssObj ) {
    if ( ! cssObj.hasOwnProperty( cssSelector ) ) {
      continue;
    }
    ...
    // loop through selector's css properties, if they exist
    if ( typeof cssObj[cssSelector] != "object" ) {
      throw "expected css property '" + cssObj[cssSelector] + "'  to be an object";
    }

    for ( cssProperty in cssObj[cssSelector] ) {
      if ( ! cssObj[cssSelector].hasOwnProperty( cssProperty ) ) {
        continue;
      }

      css += cssProperty;
      css += ":";
      css += cssObj[cssSelector][cssProperty];
      css += ";";
    }
